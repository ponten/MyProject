...
namespace RCHold
{
    public partial class fMain : Form
    {
        ...
        public void SetCombDefect()
        {
            ...
            {
                Cb_DefectType.Items.Add(dsTemp.Tables[0].Rows[i]["DEFECT_TYPE_DESC"].ToString());
            }

            Cb_DefectType.SelectedIndex = 0;
        }

        public void SetCombRCSN()
        {
            Cb_RCSN.Items.Add(SajetCommon.SetLanguage("RC_NO"));
            // 2016.5.3 By Jason
            //combRCSN.Items.Add(SajetCommon.SetLanguage("SERIAL_NUMBER"));
            // 2016.5.3 End
            Cb_RCSN.SelectedIndex = 0;
        }
        ...
        private bool CheckStatus()
        {
            ...
            {
                IsChecked = Convert.ToBoolean(Dgv_RC.Rows[i].Cells[0].FormattedValue);

                if (IsChecked)
                {
                    sCurrent_Status = Dgv_RC.Rows[i].Cells[4].Value.ToString();
...
";
            ...
            {
                new object[] { ParameterDirection.Input, OracleType.VarChar, "PARAM_0", "" },
            };

            if (!string.IsNullOrWhiteSpace(TB_OPTION1.Text))
            {
                s += " AND (UPPER(PART_NO) LIKE :PARAM_1 || '%' OR UPPER(OPTION2) LIKE :PARAM_1 || '%') ";
                p.Add(new object[] { ParameterDirection.Input, OracleType.VarChar, "PARAM_1", TB_OPTION1.Text.Trim().ToUpper() });
            }
                ...
                hiddenColumns: h))
            {
                ...
                f.Text = SajetCommon.SetLanguage("PART_NO");

                if (f.ShowDialog() == DialogResult.OK)
                {
                    DataRow result = f.ResultSets[0];

                    TB_OPTION1.Text = result["PART_NO"].ToString();

                    g_s_part_id = result["PART_ID"].ToString();
                }
        ...
        private void Excute_button_Click(object sender, EventArgs e)
        {
            ...
            string sSQL = @" SELECT COUNT(*) AS ROWCOUNT,MAX(LPAD(SUBSTR(HOLD_NO,-3,3) + 1,3,0)) AS HOLD_END
                              ...
                              WHERE HOLD_NO LIKE '" + Tb_Line_1.Text + "-" + sDB_Time + "-%'";
            dsTemp = ClientUtils.ExecuteSQL(sSQL);
            ...
            if (dsTemp.Tables[0].Rows[0]["ROWCOUNT"].ToString() != "0")
            {
                ...
                sKey = Tb_Line_1.Text + "-" + sDB_Time + "-" + dsTemp.Tables[0].Rows[0]["HOLD_END"].ToString();
            }
            else
            {
                sKey = Tb_Line_1.Text + "-" + sDB_Time + "-001";
            }
            // 2016.5.2 End

            // 2016.4.20 By Jason (防呆)
            //sHoldDesc = HD_richTextBox.Text;
            sHoldDesc = Rtb_HoldDesc.Text.Trim();
            // 2016.4.20 End

            int iDefectType = Cb_DefectType.SelectedIndex;
            decimal iTotalQty = 0;
            ...
            for (int i = 0; i < Dgv_RC.Rows.Count; i++)
            {
                ...
                if (IsChecked)
                {
                    ...
                    if (Cb_RCSN.SelectedIndex == 0)
                    {
                        sRC = Dgv_RC.Rows[i].Cells[1].Value.ToString();
                        sSerial_Number = "N/A";
                    }
                    else
                    {
                        sSerial_Number = Dgv_RC.Rows[i].Cells[1].Value.ToString();
                        sRC = "N/A";
                    }
                    ...
                    if (Cb_RCSN.SelectedIndex == 0)
                    {
                        sSQL = @"SELECT a.PROCESS_ID,a.CURRENT_STATUS FROM SAJET.G_RC_STATUS a 
                                                WHERE a.RC_NO = '" + sRC + "' AND A.RELEASE ='Y' ";
                        dsTemp = ClientUtils.ExecuteSQL(sSQL);
                        ...
                        sSQL = @"Update sajet.g_rc_status
                                set CURRENT_STATUS = '" + sCurrent_Status + "',TRAVEL_ID = " + lTravel_Id + ",UPDATE_TIME = :UPDATE_TIME,UPDATE_USERID = :UPDATE_USERID,RELEASE = 'Y' WHERE RC_NO = :RC_NO";
                        ...
                        if (sCurrent_Status == "11")
                        {
                            ...
                            DParams3[1] = new object[] { ParameterDirection.Input, OracleType.VarChar, "DEFECT_DESC", Cb_DefectType.Text };
                    ...
                    //获取QTY值
                    string sQTY = Dgv_RC.Rows[i].Cells[3].Value.ToString();
            ...
            //获取DEFECT_TYPE_ID
            string sDefectDESC = Cb_DefectType.SelectedItem.ToString();
            ...
            if (Cb_RCSN.SelectedIndex == 0)
            {
                ...
                sSQL = @"SELECT A.RC_NO,D.PROCESS_NAME,C.CURRENT_QTY
                           FROM SAJET.G_RC_HOLD_DETAIL A,
                                ...
                                SAJET.G_RC_STATUS C,
                                SAJET.SYS_PROCESS D
                          WHERE A.HOLD_NO = '" + sKey + "' AND A.HOLD_NO = B.HOLD_NO AND A.RC_NO = C.RC_NO AND A.PROCESS_ID = D.PROCESS_ID AND C.RELEASE = 'Y' ";
                // 2016.4.20 End
                dsTemp = ClientUtils.ExecuteSQL(sSQL);
            }
            else
        ...
        {
            //判断查询条件
            if (!string.IsNullOrEmpty(Tb_RCSN.Text))
            {
                switch (Cb_RCSN.SelectedIndex)
                {
                    case 0:
                        sSQL = @"SELECT A.RC_NO,SAJET.SJ_RC_STATUS(A.CURRENT_STATUS) RC_STATUS,
                            ...
                            AND A.RELEASE = 'Y'
                            AND E.WO_STATUS = '3'
                            AND A.RC_NO LIKE '%" + Tb_RCSN.Text + "%'";

                        break;
                    default:
                        sSQL = @"SELECT RC_NO FROM SAJET.G_SN_STATUS WHERE SERIAL_NUMBER LIKE '%" + Tb_RCSN.Text + "%'";
                        dsTemp = ClientUtils.ExecuteSQL(sSQL);
                        if (dsTemp.Tables[0].Rows.Count == 0)
                            return;
                        ...
                        if (sRC == "N/A")
                        {
                            sSQL = @"SELECT A.SERIAL_NUMBER,SAJET.SJ_RC_STATUS(A.CURRENT_STATUS) RC_STATUS,
                                                ...
                                                AND A.PART_ID = C.PART_ID 
                                                AND A.CURRENT_STATUS in(0,1)
                                                AND A.SERIAL_NUMBER LIKE'%" + Tb_RCSN.Text + "%'";
                        }
                        else
                        {
            ...
            if (Tb_Line_2.Text != "")
            {
                ...
                sSQL = sSQL + " AND D.PDLINE_ID = " + Tb_Line_2.Text;
            ...
            if (Tb_WO.Text.Trim() != "")
            {
                sSQL = sSQL + " AND UPPER(a.work_order) like UPPER('%" + Tb_WO.Text + "%')";//模糊查询
            }
            ...
            if (Cb_RCSN.SelectedIndex == 0)
            {
                ...
                for (int i = 0; i < dsTemp.Tables[0].Rows.Count; i++)
                {
                    ...
                    Dgv_RC.Rows[i].Cells[1].Value = dsTemp.Tables[0].Rows[i]["RC_NO"];
                    Dgv_RC.Rows[i].Cells[2].Value = dsTemp.Tables[0].Rows[i]["PROCESS_NAME"];
                    Dgv_RC.Rows[i].Cells[3].Value = dsTemp.Tables[0].Rows[i]["CURRENT_QTY"];
                    Dgv_RC.Rows[i].Cells[4].Value = dsTemp.Tables[0].Rows[i]["RC_STATUS"];
                }
            }
            else
            {
                Dgv_RC.Columns[1].HeaderText = SajetCommon.SetLanguage("SERIAL_NUMBER");

                for (int i = 0; i < dsTemp.Tables[0].Rows.Count; i++)
                {
                    Dgv_RC.Rows.Add();
                    Dgv_RC.Rows[i].Cells[0].Value = false;
                    Dgv_RC.Rows[i].Cells[1].Value = dsTemp.Tables[0].Rows[i]["SERIAL_NUMBER"];
                    Dgv_RC.Rows[i].Cells[2].Value = dsTemp.Tables[0].Rows[i]["PROCESS_NAME"];
                    Dgv_RC.Rows[i].Cells[3].Value = dsTemp.Tables[0].Rows[i]["GOOD_QTY"];
                    Dgv_RC.Rows[i].Cells[4].Value = dsTemp.Tables[0].Rows[i]["RC_STATUS"];
                }
            }
        }
