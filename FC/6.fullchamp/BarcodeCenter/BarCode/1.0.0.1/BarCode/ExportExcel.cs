...
{
    //利用xml格式方式來Export成Excel,速度快,不會啟動Excel
    public class CreateExcel
    {
        ...
        public void ExportToExcel(DataGridView GridView)
        {
            ...
            string[] sColumnList2 = new string[GridView.Columns.Count];
            for (int i = 0; i <= GridView.Columns.Count - 1; i++)
            {
                ...
                if (GridView.Columns[i].Visible)
                {
                    ...
                    sColumnList2[index] = GridView.Columns[i].Name;
                }
            }
            ...
            dsGrid.Tables.Add();
            for (int i = 0; i <= sColumnList2.Length - 1; i++)
            {
   //             MessageBox.Show(GridView.Columns[sColumnList2[i].ToString()].HeaderText);
                dsGrid.Tables[0].Columns.Add(GridView.Columns[sColumnList2[i].ToString()].HeaderText);
            }
            ...
            for (int i = 0; i <= GridView.Rows.Count - 1; i++)
            {
                ...
                int iCol = -1;
                for (int j = 0; j <= sColumnList2.Length - 1; j++)
                {
                    iCol = iCol + 1;
                    dsGrid.Tables[0].Rows[i][iCol] = GridView.Rows[i].Cells[sColumnList2[j].ToString()].Value;
                }
            }
            ExportToExcel(dsGrid);
        }
        public void ExportToExcel(DataTable table)//只是將DataTable存到DataSet裡
        {
        ...
        {
            XmlDocument xml = CreateFile();//宣告XML
            XmlNode nodeworkbook = xml.SelectSingleNode("Workbook");//用來當WorkBook的Node


            for (int k = 0; k < ds.Tables.Count; k++)//依Table各數匯出Sheet
            {
