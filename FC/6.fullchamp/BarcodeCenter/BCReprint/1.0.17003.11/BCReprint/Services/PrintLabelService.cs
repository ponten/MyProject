...
namespace BCReprintDll.Services
{
    internal static class PrintLabelService
    {
        ...
        /// 基礎檔案路徑
        /// </summary>
        private static string BasePath = Directory.GetCurrentDirectory().Contains("BarcodeCenter") ?
...
";
        ...
        /// <param name="message"></param>
        /// <returns></returns>
        internal static bool PrintLabel(List<string> rc_list, out string message)
        {
            message = "OK";
            ...
            foreach (string rc_no in rc_list)
            {
                string s_temp = string.Empty;

                s_temp += GetPrintContent(rc_no, LabelDataGroupEnum.WORK_ORDER);

                s_temp += GetPrintContent(rc_no, LabelDataGroupEnum.RC_NO);

                s_temp += GetPrintContent(rc_no, LabelDataGroupEnum.B050);

                s_temp += GetPrintContent(rc_no, LabelDataGroupEnum.B060);

                s_temp += GetPrintContent(rc_no, LabelDataGroupEnum.B070);
