    ...
    public partial class fMain : Form
    {
        ...
        private void ShowGanttChart(TimeScale timeScale)
        {
            ...
            foreach (DataGridViewRow row in gvData.Rows)
            {
                ...
                task.Name = row.Cells["WORK_ORDER"].Value.ToString();
        ...
        private void UpdateDateTimeOnUi(DateTime earliestDateTime, DateTime latestDateTime)
        {
            datePickerFrom.Value = earliestDateTime;
            datePickerTo.Value = latestDateTime;
            datePickerFrom.Checked = true; // update the date on UI
        ...
        private void fMain_Load(object sender, EventArgs e)
        {
...

            {
                ...
                TDBInitial dbInitial = new TDBInitial();
                dbInitial.sValue = "WORK_ORDER,WOSTATUS,PART_NO,VERSION,WO_RULE," +
                                   ...
                                   "RULE1,OLD_NUMBER,UNIT,BLUEPRINT,REMARK";

                dbInitial.slValue = new List<string>();
                dbInitial.slValue.AddRange(new string[]
                {
                    "WORK_ORDER", "WOSTATUS", "PART_NO", "VERSION", "WO_RULE",
                    "WO_TYPE", "TARGET_QTY", "PDLINE_NAME", "ROUTE_NAME", "MADE_CATEGORY", "CUSTOMER1",
                    "CUSTOMER3", "SALES", "CUSTOMIZE", "PRODUCT_RATE", "OUTSOURCE",
                    "CUSTOMER_DATE", "CUSTOMER_DUE_DATE", "PALLETS", "WO_SCHEDULE_DATE", "WO_DUE_DATE",
                    "RULE1", "OLD_NUMBER", "UNIT", "BLUEPRINT", "REMARK"
            ...
            for (int i = 0; i < TableDefine.tGridField.Length; i++)
            {
                ...
                {
                    lablTemp = new Label();
                    lablTemp.Font = new Font("新細明體", 11);
                    ...
                    else
                    {
                        ...
                        txtTemp.Font = new Font("新細明體", 11, FontStyle.Bold);
        ...
        public void ShowData(bool? isSerachButtonClicked = null)
        {
            string sSQL;
...
";
                ...
                foreach (DataRow dr in dsGRcTravel.Tables[0].Rows)
                {
                    ...
                    if (workOrderIdExist)
                    {
                        var grcTravel = new GRcTravel()
                        {
                            ...
                            RcNo = int.Parse(dr[3].ToString()),
                            NgQty = int.Parse(dr[4].ToString())
                        };
        ...
        private void gvData_CellValueNeeded(object sender, DataGridViewCellValueEventArgs e)
        {
            try
            {
                // ColumnIndex = 16 is CUSTOMER_DATE,  ColumnIndex = 17 is CUSTOMER_DUE_DATE
                // ColumnIndex = 19 is WO_SCHEDULE_DATE,  ColumnIndex = 20 is WO_DUE_DATE
                var str = memoryCache.RetrieveElement(e.RowIndex, e.ColumnIndex);

                if (!string.IsNullOrEmpty(str) &&
                    ((e.ColumnIndex == 16 || e.ColumnIndex == 17 ||
                      e.ColumnIndex == 19 || e.ColumnIndex == 20)))
                {
                    e.Value = DateTime.Parse(str).ToString("yyyy/MM/dd");
                }
                else if (!string.IsNullOrEmpty(str) && e.ColumnIndex == 9) // Made_Category
                {
        ...
        private void btnRelease_Click(object sender, EventArgs e)
        {
            ...
            string sMsg = SajetCommon.SetLanguage("Change Work Order Status to Release");
