...
";
        ...
        private void btnTemp_Click(object sender, EventArgs e)
        {
            ...
            fFilter.sSQL = button.Tag.ToString();
...
";
        ...
        {
            ClientUtils.ExecuteSQL("DELETE FROM SAJET.G_PACK_SPEC WHERE WORK_ORDER = :WORK_ORDER", new object[1][]
            {
                new object[4]
                {
                    ParameterDirection.Input,
            ...
            if (g_DBInitial.ContainsKey("Packing Spec Part"))
            {
                ...
                for (int index1 = 0; index1 <= LVPkSPec.Items.Count - 1; ++index1)
                {
                    objArray[4] = new object[4]
                    {
                        ...
                        OracleType.VarChar,
                        "TPKSPEC",
                        LVPkSPec.Items[index1].Tag.ToString()
                    };
                    for (int index2 = 1; index2 < LVPkSPec.Columns.Count; ++index2)
                        objArray[4 + index2] = new object[4]
                        {
                            ParameterDirection.Input,
                            OracleType.VarChar,
                            "T" + LVPkSPec.Columns[index2].Name,
                            LVPkSPec.Items[index1].SubItems[index2].Text.ToString()
                        };
        ...
        {
            ClientUtils.ExecuteSQL("DELETE FROM SAJET.G_WO_BOM WHERE WORK_ORDER = :WORK_ORDER", new object[1][]
            {
                new object[4]
                {
                   ParameterDirection.Input,
            ...
            if (dataSet.Tables[0].Rows.Count > 0)
            {
                ...
                ClientUtils.ExecuteSQL(
                    "INSERT INTO SAJET.G_WO_BOM \r\n                    (WORK_ORDER, PART_ID, ITEM_PART_ID, ITEM_GROUP, ITEM_COUNT, PROCESS_ID, VERSION, UPDATE_USERID) \r\n                    SELECT :WORK_ORDER, :PART_ID,ITEM_PART_ID, ITEM_GROUP, ITEM_COUNT, PROCESS_ID, VERSION, :EMP_ID\r\n                    from SAJET.SYS_BOM \r\n                    WHERE BOM_ID = :BOM_ID \r\n                    And Enabled='Y' ",
                    new object[4][]
                    {
                        new object[4]
                        {
