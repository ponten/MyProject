...
namespace Braincase.GanttChart
{
    ...
    public partial class Chart : UserControl
    {
        ...
        public Chart()
        {
            ...
            viewport.WheelDelta = BarSpacing * 10;
            _mViewport = viewport;
            ...
            TaskFormat = new GanttChart.TaskFormat() {
                Color = Brushes.Black,
                Border = Pens.Maroon,
                BackFill = Brushes.Green,//Brushes.MediumSlateBlue,
            ...
            RealTaskFormat = new GanttChart.TaskFormat() {
                Color = Brushes.Black,
                Border = Pens.Maroon,
                BackFill = Brushes.Red,//Brushes.Crimson,
                ForeFill = Brushes.YellowGreen,
                SlackFill = new System.Drawing.Drawing2D.HatchBrush(System.Drawing.Drawing2D.HatchStyle.LightDownwardDiagonal, Color.Red, Color.Transparent)
            };
        ...
        /// </summary>
        public TaskFormat RealTaskFormat { get; set; }
        ...
        public void SetToolTip(Task task, string text)
        {
            ...
            {
                if (!_mTaskToolTip.ContainsKey(task.Name))
                {
                    _mTaskToolTip[task.Name] = text;
                }
            }
        }
        ...
        {
            if (task != null)
                return _mTaskToolTip[task.Name];
            else
                return string.Empty;
        }
        ...
        {
            if(task != null)
                _mTaskToolTip.Remove(task.Name);
        }
        ...
        protected virtual void OnTaskMouseOver(TaskMouseEventArgs e)
        {
            ...
            if (_mTaskToolTip.ContainsKey(task.Name))
            {
                _mOverlay.ShowToolTip(_mViewport.DeviceToWorldCoord(e.Location), _mTaskToolTip[task.Name]);
                this.Invalidate();
            }
        }
        /// <summary>
        ...
        class ChartOverlay
        {
            public void Paint(ChartPaintEventArgs e)
            {
                ...
                if (_mToolTipMouse != Point.Empty && _mToolTipText != string.Empty)
                {
                    ...
                    //tooltiprect.Inflate(1, 1);
                    //float y = e.Chart._ChartRowToChartCoord(Row) + e.Chart.BarHeight / 2.0f;
                    float y = e.Chart.BarHeight / 2.0f;
                    var offset = 0;
                    if (y > tooltiprect.Y)
                    {
                        offset = 100;
                    ...
                    g.FillRectangle(Brushes.LightYellow, tooltiprect.X, tooltiprect.Y + offset, tooltiprect.Width, tooltiprect.Height);
                    g.DrawString(_mToolTipText, chart.Font, Brushes.Black, textstart.X, textstart.Y + offset);
                }
            }
        ...
        private void _GenerateModels()
        {
            ...
            foreach (var task in _mProject.Tasks)
            {
                ...
                { 
                    if (row == 0)
                    {
                        y_coord = this.HeaderTwoHeight + this.HeaderOneHeight + (this.BarSpacing - this.BarHeight) / 2;
                    }
                    else
                    {
                        y_coord += this.BarSpacing;
                    }

                    // clone task
                    var taskColoned = new Task()
                    {
                        IsCloned = true,
                        Name = task.Name,
                        ...
                        RunCardNo = task.RunCardNo,
                        NgQty = task.NgQty,
                    };

                    // Compute task rectangle
                    var taskRect = new RectangleF(task.Start * this.BarWidth + this.BarWidth / 2, y_coord, task.Duration * this.BarWidth, (int)BarHeight);
                    var taskRect_2 = new RectangleF(task.RealStart * this.BarWidth + this.BarWidth / 2, y_coord+3, task.RealDuration * this.BarWidth, (int)BarHeight/2);

                    _mChartTaskRects.Add(task, taskRect);
                    _mChartTaskRects.Add(taskColoned, taskRect_2);

                    _mChartTaskHitRects.Add(task, taskRect);
                    _mChartTaskHitRects.Add(taskColoned, taskRect_2);

                    // Find maximum end time
                    var tempTaskEnd = task.End > task.RealEnd ? task.End : task.RealEnd;
                    if (tempTaskEnd > end) end = tempTaskEnd;
        ...
        private void __DrawHeaderOne(Graphics graphics, int columnIndex, HeaderPaintEventArgs e, RectangleF h2textrect)
        {
            ...
            else if (this.TimeScaleDisplay == GanttChart.TimeScaleDisplay.DayOfMonth || this.TimeScaleDisplay == GanttChart.TimeScaleDisplay.WeekOfYear)
            {
                ...
                if (columnIndex == 0 || (date.Month != info.Dates[columnIndex - 1].Month))
                {
                    ...
                    var h1label = date.ToString("yyyy/MM"); // set the Header datatime format (ex: 2017/03)
        ...
        private int _DrawTasks(Graphics graphics, Rectangle clipRect)
        {
            ...
            foreach (var task in _mChartTaskRects.Keys)
            {
                ...
                if (taskrect.Left <= viewRect.Right)
                {
                    ...
                    if (task.IsCloned)
                    {
                        e = new TaskPaintEventArgs(graphics, clipRect, this, task, row, false, this.Font, this.RealTaskFormat);
                    }
                    else
                    {
                    ...
                    // write text
                    var taskName = GetCorrectTaskName(task);
                    if (this.ShowTaskLabels && taskName != string.Empty)
                    {
                        // write name to the end of the task/dummy-task
                        var txtrect = _TextAlignLeftMiddle(graphics, taskrect, taskName, e.Font, labelMargin);
                        txtrect.Offset(taskrect.Width, 0);
                        if (viewRect.IntersectsWith(txtrect))
                        {
                            graphics.DrawString(taskName, e.Font, e.Format.Color, txtrect); 
        ...
        private static string GetCorrectTaskName(Task task)
        {
            string taskName;
            if (task.RealEnd > task.End)
            {
                if (!task.IsCloned)
                {
                    taskName = string.Empty; // hide task name
                }
                else
                {
                    taskName = task.Name; // display cloned-task name
                }
            }
            else
            {
                if (task.IsCloned)
                {
                    taskName = string.Empty; // hide cloned-task name
                }
                else
                {
                    taskName = task.Name; // display task name
                }
            }
        ...
        Dictionary<string, string> _mTaskToolTip = new Dictionary<string, string>(); // key: task name, value: tooltip text
