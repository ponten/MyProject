...
";
                ...
                if (!dataCurrentRow.Cells["PART_ID"].Value.ToString().Equals(""))
                {
                    ...
                    //SajetClass.SajetCommon.Show_Message("2", -1);
                    combRoute.Items.Add("");
...
";
        ...
        private void fData_Shown(object sender, EventArgs e)
        {
            ...
            if (combLine.Items.Count > 1 && combLine.SelectedIndex < 0)
            {
                combLine.SelectedIndex = 1;
            }
            if (combWoType.Items.Count > 1 && combWoType.SelectedIndex < 0)
            {
...
";
               ...
               //victor modify 變更途程
                ListViewProduceDetail(editPart.Text, combRoute.SelectedItem.ToString());
            }
            finally
            {
...
";
        ...
        private void SetPalletVersionRouteOldNoUnitBlueprint()
        {
            ...
            if (!string.IsNullOrEmpty(editPart.Text))
            {
                ...
                string sql = $@" SELECT OPTION1 PALLETS,VERSION, B.ROUTE_NAME, A.OPTION2, A.UPC, A.OPTION4
                                 FROM SAJET.SYS_PART A
                                 JOIN SAJET.SYS_PART_ROUTE C
                                   ON A.PART_ID = C.PART_ID
                                 LEFT JOIN SAJET.SYS_RC_ROUTE B
                                   ON C.ROUTE_ID = B.ROUTE_ID
                                WHERE PART_NO='{editPart.Text}'";
                ...
                foreach (DataRow row in dataSet.Tables[0].Rows)
                {
                    ...
                    //SajetClass.SajetCommon.Show_Message("3", -1);
...
";
        ...
        {
            combRoute.Items.Clear();
            //SajetClass.SajetCommon.Show_Message("1",-1);
            combRoute.Items.Add("");
...
WITH PART_INFO AS (
    ...
    FROM
        SAJET.SYS_PART A
        JOIN SAJET.SYS_PART_ROUTE B ON A.PART_ID=B.PART_ID
        JOIN SAJET.SYS_RC_ROUTE C ON B.ROUTE_ID=C.ROUTE_ID
    WHERE A.PART_NO = :PART_NO
     AND C.ROUTE_NAME=:ROUTE_NAME
),
/* 該料號的預設生產途程的所有製程參數 */
PART_PARAMS AS (
    SELECT
