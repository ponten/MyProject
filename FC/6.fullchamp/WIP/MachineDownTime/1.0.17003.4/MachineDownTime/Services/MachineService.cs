...
";
            ...
            out string message)
        {
            ...
            {
                // 取得時間段的結束時間，比較新紀錄的開始時間
                if (!DateTime.TryParse(down_log.Tables[0].Rows[i]["END_TIME"].ToString(), out DateTime end_time_i))
                {
                ...
                if (start_time > end_time_i)
                {
                    ...
                    if (!DateTime.TryParse(down_log.Tables[0].Rows[i + 1]["START_TIME"].ToString(), out DateTime start_time_i))
                    {
                        message = SajetCommon.SetLanguage(MessageEnum.TimeSpanCheckError.ToString()); ;
