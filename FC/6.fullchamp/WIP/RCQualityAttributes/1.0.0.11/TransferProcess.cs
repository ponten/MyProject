    ...
    public partial class TransferProcess : Form
    {
        ...
        private void TransferProcess_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dsTemp.Tables[0].Rows.Count; i++)
            {
                ...
                Btn.Tag = dsTemp.Tables[0].Rows[i]["NODE_CONTENT"].ToString();
                Btn.Text = dsTemp.Tables[0].Rows[i]["PROCESS_NAME"].ToString();

                Btn.Height = Btn.Width = 90;
                Btn.BackColor = Color.Khaki;
                Btn.Font = new System.Drawing.Font("新細明體", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(134)));

                Btn.Click += new EventHandler(Btn_OK_Click);
                FLP_ButtonZone.Controls.Add(Btn);
            }

            //if (FLP_ButtonZone.Controls.Count == 1)
            //{
            Button BtnClick = FLP_ButtonZone.Controls[0] as Button;
                ...
                sSQL = @"  SELECT A.NODE_CONTENT,B.PROCESS_NAME,
                     ...
                     + @"          GROUP BY NODE_CONTENT) AS LINK_NAME
                            ...
                            WHERE A.NODE_CONTENT = B.PROCESS_ID
                              ...
                              AND A.ROUTE_ID = " + sRoute_Id
                     + "      AND A.GROUP_ID = " + sNode_Id
                     + "      AND A.NODE_CONTENT <> " + sProcess_Id
                     + " ORDER BY A.NODE_CONTENT ASC";
                dsTemp = ClientUtils.ExecuteSQL(sSQL);

                for (int i = 0; i < dsTemp.Tables[0].Rows.Count; i++)
                {
                    DGV_Process.Rows.Insert(DGV_Process.Rows.Count, new object[] { dsTemp.Tables[0].Rows[i]["NODE_ID"].ToString(), dsTemp.Tables[0].Rows[i]["NODE_CONTENT"].ToString(), dsTemp.Tables[0].Rows[i]["PROCESS_NAME"].ToString(), dsTemp.Tables[0].Rows[i]["LINK_NAME"].ToString() });
                }
            }
            else
            {
                ...
                for (int i = 0; i < dsTemp.Tables[0].Rows.Count; i++)
                {
                    if (dsTemp.Tables[0].Rows[i]["NODE_TYPE"].ToString() == "9")
                    {
                        ...
                        // (END製程)
                        DGV_Process.Rows.Insert(DGV_Process.Rows.Count, new object[] { dsTemp.Tables[0].Rows[i]["NODE_ID"].ToString(), "0", dsTemp.Tables[0].Rows[i]["NODE_CONTENT"].ToString(), dsTemp.Tables[0].Rows[i]["LINK_NAME"].ToString() });
                    }
                    ...
                    if (dsTemp.Tables[0].Rows[i]["NODE_TYPE"].ToString() == "2" || dsTemp.Tables[0].Rows[i]["NODE_TYPE"].ToString() == "3")
                    {
                        ...
                        // (GROUP製程)
                        sSQL = @"  SELECT A.NODE_ID,A.NODE_CONTENT,B.PROCESS_NAME
                                     FROM SAJET.SYS_RC_ROUTE_DETAIL A,SAJET.SYS_PROCESS B
                                    WHERE A.NODE_CONTENT = B.PROCESS_ID
                                      AND A.GROUP_ID = " + dsTemp.Tables[0].Rows[i]["NODE_ID"].ToString()
                             + " ORDER BY A.NODE_CONTENT ASC";
                        dsTempGroup = ClientUtils.ExecuteSQL(sSQL);

                        for (int j = 0; j < dsTempGroup.Tables[0].Rows.Count; j++)
                        {
                            DGV_Process.Rows.Insert(DGV_Process.Rows.Count, new object[] { dsTemp.Tables[0].Rows[i]["NODE_ID"].ToString(), dsTempGroup.Tables[0].Rows[j]["NODE_CONTENT"].ToString(), dsTempGroup.Tables[0].Rows[j]["PROCESS_NAME"].ToString(), dsTemp.Tables[0].Rows[i]["NODE_CONTENT"].ToString() });
                        }
                    }
                }

                //sSQL = @"   SELECT A.NODE_ID,A.NODE_CONTENT,C.PROCESS_NAME,B.LINK_NAME
                sSQL = @"   SELECT DISTINCT A.NODE_ID,A.NODE_CONTENT,C.PROCESS_NAME,B.LINK_NAME
                              FROM SAJET.SYS_RC_ROUTE_DETAIL A,
                                   (SELECT NEXT_NODE_ID,LINK_NAME
                                      FROM SAJET.SYS_RC_ROUTE_DETAIL
                                     WHERE ROUTE_ID = " + sRoute_Id
                     + "               AND NODE_ID = " + sNode_Id
                     + @"          ) B,SAJET.SYS_PROCESS C
                             WHERE A.NODE_ID = B.NEXT_NODE_ID
                               AND A.NODE_CONTENT = C.PROCESS_ID";

                if (sCheckEnd == "Y")
                {
                    sSQL = sSQL + " AND A.NODE_TYPE <> 9 ";
                }

                if (sCheckGroup == "Y")
                {
                    sSQL = sSQL + " AND A.NODE_TYPE <> 2 ";
                    sSQL = sSQL + " AND A.NODE_TYPE <> 3 ";
                }

                sSQL = sSQL + @" UNION ALL

                                 SELECT B.NODE_ID,A.NODE_CONTENT,C.PROCESS_NAME,B.NODE_CONTENT AS LINK_NAME
                                   FROM SAJET.SYS_RC_ROUTE_DETAIL A,
                                        (  SELECT NODE_ID,NODE_CONTENT
                                             FROM SAJET.SYS_RC_ROUTE_DETAIL
                                            WHERE ROUTE_ID = " + sRoute_Id
                          + "                 AND NODE_ID = " + sNode_Id
                          + @"           GROUP BY NODE_ID,NODE_CONTENT) B,SAJET.SYS_PROCESS C
                                  WHERE A.GROUP_ID = B.NODE_ID
                                    AND A.NODE_CONTENT = C.PROCESS_ID"
                          + "       AND A.NODE_CONTENT <> " + sProcess_Id;
