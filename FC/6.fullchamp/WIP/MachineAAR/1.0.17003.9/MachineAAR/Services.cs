                ...
                stages.ForEach(x =>
                {
                    ...
                    if (!processes.Any(z => z?.ID == x["PROCESS_ID"].ToString()))
                    {
                        processes.Add(new ProcessModel
                        {
                            ...
                            T4 = x["OPTION2"].ToString() == "Y",
                            T6 = x["OPTION3"].ToString() == "Y",
                        });
                    }
                });
            ...
            if (d != null && d.Tables[0].Rows.Count > 0)
            {
                var resultSet = d.Tables[0].Rows.Cast<DataRow>()
                    .Select(x => new RuncardModel
                    {
                        ...
                        CURRENT_QTY = int.Parse(x["current_qty"].ToString()),
...
";
                    ...
                    p.Add(new object[] { ParameterDirection.Input, OracleType.Number, $"DATE_CODE{i}", x.DATE_CODE });
