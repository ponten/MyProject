...
namespace FTransferProcess
{
    public partial class TransferProcess : Form
    {
        ...
        public TransferProcess()
        {
            ...
            Text = $"{Text} ({SajetCommon.g_sFileName} : {SajetCommon.g_sFileVersion})";
...
";
            ...
            if (next_process_list.Tables[0].Rows.Count > 0)
            {
            ...
            {
                // 如果是最後一個製程（END），要拉出來特別處理
                DataRow node_row
                    = route_detail
                    .First(x => x["NODE_ID"].ToString() == next_node_id_list[0]);

                if (node_row["NODE_TYPE"].ToString() == "9")
                {
                    var values = new object[]
                    {
                        node_row["NODE_ID"].ToString(),
                        "0",
                        "END",
                        "",
                    };
