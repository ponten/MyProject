...
";
            ...
            {
                SajetClass.SajetCommon.Show_Message(ex.ToString() + ex.StackTrace, 0);
            }
        }
...
";
        ...
        private void DtpOutDate_ValueChanged(object sender, EventArgs e)
        {
            ...
            RC_DETAILS current_rc_no = RcDetails.First(x => x.RC_NO == Current_RC_NO);

            //if (!ShareSettings)
            //{
            //    current_rc_no.OUT_PROCESS_TIME = DtpOutDate.Value;
   ...
   ,:UPDATE_USERID
        ...
        private void BtnOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //重新建立已選取 RC 的清單
                List<RC_DETAILS> ExecuteRCs
                    = CreateRCsList(DefectItems)
                    .Where(x => x.SELECT)
                    .ToList();
                ...
                foreach (var rc in ExecuteRCs)
                {
                    ...
                    int i_processed_qty = rc.PROCESSED_QTY;
                    ...
                    if (rc.Machines.Count == 1 && i_load_qty == 0)
                    {
                        rc.Machines[0].LOAD_QTY = i_current_qty - i_processed_qty;

                        i_load_qty = i_current_qty - i_processed_qty;
                    }

                    // 加工數量必須等於流程卡數量
                    if (hasProcessMachine && i_current_qty != i_load_qty + i_processed_qty)
                    {
...
";
                        ...
                        Params[1] = new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString() };
                    ...
                    Params[6] = new object[] { ParameterDirection.Input, OracleType.Number, "TGOOD", dGood };
                    Params[7] = new object[] { ParameterDirection.Input, OracleType.Number, "TSCRAP", dScrap };
                    Params[8] = new object[] { ParameterDirection.Input, OracleType.Number, "TNEXTPROCESS", rcRoute.sNext_Process };
                    Params[9] = new object[] { ParameterDirection.Input, OracleType.Number, "TNEXTNODE", rcRoute.sNext_Node };
                    Params[10] = new object[] { ParameterDirection.Input, OracleType.VarChar, "TNEXTSHEET", rcRoute.sSheet_Name };
                    Params[11] = new object[] { ParameterDirection.Input, OracleType.Number, "TBONUS", sBonus };
        ...
        private void DspReportData()
        {
            ...
            Params[0] = new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString() };
            Params[1] = new object[] { ParameterDirection.Input, OracleType.Number, "PART_ID", programInfo.dsRC.Tables[0].Rows[0]["PART_ID"].ToString() };
            ...
            if (bOneSheet)
            {
                ...
                //Params[1] = new object[] { ParameterDirection.Input, OracleType.Number, "PART_ID", programInfo.dsRC.Tables[0].Rows[0]["WO_OPTION2"].ToString() };
...
SELECT
    ...
    0 ""BONUS"",
    1 ""WORK_HOUR"",
    NVL(D.PROCESSED_QTY, 0) ""PROCESSED_QTY""
FROM
    SAJET.G_RC_STATUS   A,
    RUNCARD_FILTER      B,
    RUNCARD_STATUS      C,
...
";
                object[][] Params = new object[3][];
                Params[0] = new object[] { ParameterDirection.Input, OracleType.Number, "ROUTE_ID", rcRoute.g_sRouteID };
                Params[1] = new object[] { ParameterDirection.Input, OracleType.VarChar, "NODE_CONTENT", rcRoute.g_sProcessID };
                Params[2] = new object[] { ParameterDirection.Input, OracleType.Number, "NODE_ID", rcRoute.sNode_Id };
            ...
            try
            {
                ...
                if (DgvBOM.CurrentRow.Cells["PURCHASE"].Value.ToString() == "Y")  // 不為外購要檢查料件序號規則
                {
                    ...
                    else
                    {
                        ...
                        Params[0] = new object[] { ParameterDirection.Input, OracleType.VarChar, "RC_NO", sRC };
                        Params[1] = new object[] { ParameterDirection.Input, OracleType.Number, "PART_ID", programInfo.dsRC.Tables[0].Rows[0]["PART_ID"].ToString() };
                        Params[2] = new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString() };
        ...
        private List<RC_DETAILS> CreateRCsList(List<Defect> defects)
        {
            ...
            var temp = (from row in RCSource.Tables[0].Rows.Cast<DataRow>()
                        select new RC_DETAILS
                        {
                            ...
                            PROCESSED_QTY = int.Parse(row["PROCESSED_QTY"].ToString()),
...
";
        ...
        private void btnSplit_Click(object sender, EventArgs e)
        {
            ...
            string[] saRC = DgvRC.Rows.Cast<DataGridViewRow>()
                .Where(c => Convert.ToBoolean(c.Cells[CHECK.Name].FormattedValue))
                .Select(c => c.Cells[RC_NO.Name].Value.ToString())
                .ToArray();
            if (saRC.Count() != 1)
            {
                SajetCommon.Show_Message("Please Select a RC", 0);
                return;
            }
        ...
        private void btnMerge_Click(object sender, EventArgs e)
        {
            ...
            if (saRC.Count() < 2)
            {
                SajetCommon.Show_Message("Please select more than 1 RC", 0);
                return;
            }
            //檢查選取的是否為同個母批

            for (int i = 1; i < saRC.Count(); i++)
                if (saRC[i].Split('-')[0] != saRC[0].Split('-')[0])
                {
                    SajetCommon.Show_Message("Please choose RCs form the same source RC", 0);
                    return;
                }

            RCMerge.fMain f = new RCMerge.fMain();
            f.editRC.Text = saRC[0];
            f.lsRcToMerge = saRC.ToList();
            f.ShowDialog();
