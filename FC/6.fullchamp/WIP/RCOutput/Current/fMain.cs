...
";
            ...
            sSQL = @"SELECT WORK_ORDER,PROCESS_ID FROM SAJET.G_RC_STATUS
    WHERE RC_NO=:rc_no ";
            Params = new List<object[]>
            {
                new object[] { ParameterDirection.Input, OracleType.VarChar, "rc_no", txtInput.Text },
            ...
            if (dsTemp != null &&
                dsTemp.Tables[0].Rows.Count > 0)
            {
                string sWork_order = dsTemp.Tables[0].Rows[0]["WORK_ORDER"].ToString();
                string sPorcess_id = dsTemp.Tables[0].Rows[0]["PROCESS_ID"].ToString();
                Tb_OP_count.Text = OtSrv.GetOPCount(sWork_order, sPorcess_id);
            }
            #endregion
...
";
                long l_process_id = long.Parse(programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString().Trim());

                Params = new List<object[]>
                {
                    new object[] { ParameterDirection.Input, OracleType.VarChar, "RC_NO", txtInput.Text.Trim() },
                    new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", l_process_id }
                };
            ...
            if (CbT4CheckBOM.Checked)
            {
                ...
                {
                    if (!T4Srv.Is_TheBom_OfThePartNo_OfTheRuncards_Matches(parentRC: txtInput.Text.ToUpper(), childRC: t4_rc.RC_NO_T4))
                    {
            ...
            Params = new List<object[]>
            {
                ...
                new object[] { ParameterDirection.Input, OracleType.Number, "TGOOD", programInfo.txtGood.Text.Trim() },
                new object[] { ParameterDirection.Input, OracleType.Number, "TSCRAP", programInfo.txtScrap.Text.Trim() },
                new object[] { ParameterDirection.Input, OracleType.Number, "TNEXTPROCESS", rcRoute.sNext_Process.Trim() },
                new object[] { ParameterDirection.Input, OracleType.Number, "TNEXTNODE", rcRoute.sNext_Node.Trim() },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "TNEXTSHEET", rcRoute.sSheet_Name },
                new object[] { ParameterDirection.Input, OracleType.Number, "TBONUS", TbBonus.Text.Trim() },
        ...
        private void BtnAddT4_Click(object sender, EventArgs e)
        {
            ...
            string T4Runcard = TbRcT4.Text.ToUpper().Trim();//75版版轉大寫
...
";
                ...
                Params.Add(new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString() });
            ...
            if (bOneSheet)
            {
                ...
                string s_part_id = programInfo.dsRC.Tables[0].Rows[0]["PART_ID"].ToString().Trim();

                long l_part_id = long.Parse(s_part_id);

                Params = new List<object[]>
                {
                    new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", programInfo.dsRC.Tables[0].Rows[0]["PROCESS_ID"].ToString() },
                    //new object[] { ParameterDirection.Input, OracleType.VarChar, "PART_ID", programInfo.dsRC.Tables[0].Rows[0]["PART_ID"].ToString() },
                    new object[] { ParameterDirection.Input, OracleType.Number, "PART_ID", l_part_id }
                };
...
";
                var Params = new List<object[]>
                {
                    new object[] { ParameterDirection.Input, OracleType.Number, "ROUTE_ID", rcRoute.g_sRouteID },
                    new object[] { ParameterDirection.Input, OracleType.VarChar, "NODE_CONTENT", rcRoute.g_sProcessID },
                    new object[] { ParameterDirection.Input, OracleType.Number, "NODE_ID", rcRoute.sNode_Id }
                };
...
";
                    p = new List<object[]>
                    {
                        new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", rcRoute.sNext_Process }
                    };
...
";
            var p = new List<object[]>
            {
                new object[] { ParameterDirection.Input, OracleType.VarChar, "RC_NO", RC }
            };
...
";
        ...
        private void ClearData()
        {
            ...
            try { DgvMachine.Rows.Clear(); } catch { DgvMachine.DataSource = null; }
        ...
        private void btnSplit_Click(object sender, EventArgs e)
        {
            ...
            f.sRC_CallByOthers = RC_NO_INFO["RC_NO"].ToString();
            f.ShowDialog();
            if (f.DialogResult == DialogResult.OK)
            {
                RC_NO_INFO = RcSrv.GetRcNoInfo(RC_NO_INFO["RC_NO"].ToString());
                CheckInput();
                ShowShiftQTY();
            }
