                    ...
                    case "FIELD":
                        {
                            ...
                            {
                                slValue
                                    = ("RC_NO,WORK_ORDER,PART_NO,VERSION,SPEC1,GOOD_QTY,PROCESSED_QTY,REMARK")
                                    .Split(',');
                            }
                            else if (iCount == 1)
                            {
                                slValue
                                    = ("QTY,ROUTE_NAME,FACTORY_CODE,PROCESS_NAME,SPEC2,SCRAP_QTY,REMAINED_QTY")
                                    .Split(',');
                            }
                            else
                            {
                    ...
                    case "LABEL":
                        {
                            ...
                            {
                                slValue
                                    = ("流程卡,工單,料號,版本,品名,良品數,已加工數量,備註")
                                    .Split(',');
                            }
                            else if (iCount == 1)
                            {
                                slValue
                                    = ("流程卡數量,途程名稱,廠別代碼,製程名稱,規格,不良品數,剩餘數量")
                                    .Split(',');
                            }
                            else
                            {
...
";
            ...
            if (qc_items_count > 0)
            {
                if (need_qc || must_inspect)
                {
                    ...
                    int _tableRow = tableLayoutPanel1.RowCount - 1;
...
";
            ...
            #region 強調顯示 舊編(OPTION2) 藍圖(OPTION4) 1.0.17003.19
            sSQL = $@"
SELECT
    A.OPTION2 FORMER_NO
   ,A.OPTION4 BLUEPRINT
...
";
            ...
            tsLabelBluePrint.Text = SajetCommon.SetLanguage("Blueprint") + ": ";

            tslabelFormerNO.Text = SajetCommon.SetLanguage("Former NO") + ": ";

            if (dsTemp != null && dsTemp.Tables[0].Rows.Count > 0)
            {
                if (!string.IsNullOrWhiteSpace(dsTemp.Tables[0].Rows[0]["FORMER_NO"].ToString()))
                {
                    tslabelFormerNO.Text += dsTemp.Tables[0].Rows[0]["FORMER_NO"].ToString();
                }
                else
                {
                    tslabelFormerNO.Text += " -- ";
                }

                if (!string.IsNullOrWhiteSpace(dsTemp.Tables[0].Rows[0]["BLUEPRINT"].ToString()))
                {
                    tsLabelBluePrint.Text += dsTemp.Tables[0].Rows[0]["BLUEPRINT"].ToString();
                }
                else
                {
                    tsLabelBluePrint.Text += " -- ";
                }
            }
            else
            {
...
";
        ...
        private void FMain_Shown(object sender, EventArgs e)
        {
            ...
            var new_font = new Font("新細明體", 20, FontStyle.Bold);
            ...
            var emphasis_font = new Font("新細明體", 12, FontStyle.Bold);
...
";
        ...
        private void BtnOK_Click(object sender, EventArgs e)
        {
            ...
            if (DgvMachine.Rows.Count > 0) // 檢查是否有勾選
            {
                ...
                {
                    sMachine += DgvMachine.Rows[i].Cells[nameof(mACHINECODEDataGridViewTextBoxColumn)].EditedFormattedValue.ToString() + (char)9;
            ...
            if (machines.Count == 1 && machines.Any(x => x.LOAD_QTY <= 0))
            {
            ...
            {
                // 機台代碼
                string machineCode = row.Cells[nameof(mACHINECODEDataGridViewTextBoxColumn)].Value.ToString();

                // 負載量
                total_load_qty += int.Parse(row.Cells[nameof(lOADQTYDataGridViewTextBoxColumn)].Value.ToString());

                // 日期碼
                string date_code = row.Cells[nameof(dATECODEDataGridViewTextBoxColumn)]
                    .EditedFormattedValue?.ToString().Trim() ?? "";

                // 爐序號
                string stoveSeq = row.Cells[nameof(sTOVESEQDataGridViewTextBoxColumn)]
                ...
                if (usingT4OrT6stove || !string.IsNullOrEmpty(date_code))
                {
                    ...
                    {
                        // 6 碼檢查
                        if (date_code.Length == 6
                            && !DateTime.TryParseExact(date_code, "yyMMdd", null,
                            System.Globalization.DateTimeStyles.None, out DateTime d1))
                        {
                            dateCodeNotPass.Add(machineCode);
                        }
                        // 8 碼檢查
                        else if (date_code.Length == 8
                            && !DateTime.TryParseExact(date_code, "yyyyMMdd", null,
                ...
                if (usingT4OrT6stove &&
                    string.IsNullOrWhiteSpace(stoveSeq))
                {
                    stoveSeqNotSet.Add(machineCode);
                }

                if (string.IsNullOrWhiteSpace(date_code) ^ string.IsNullOrWhiteSpace(stoveSeq))
                {
                    notCollectTwo.Add(machineCode);
                }
            }

            if (stoveSeqNotSet.Any())
            {
                message
                    = SajetCommon.SetLanguage("Must select stove sequence")
                    + Environment.NewLine
                    + SajetCommon.SetLanguage("Machine")
                    + ": "
                    + string.Join(", ", stoveSeqNotSet)
                    ;
            ...
            {
                message = SajetCommon.SetLanguage("DateCode and stove sequence should be collected at the same time");
            ...
            if (dateCodeNotPass.Any())
            {
                ...
                if (usingT4OrT6stove)
                {
                    message
                        += Environment.NewLine
                        + SajetCommon.SetLanguage("Each machine must record the dateCode");
                }
            ...
            if (usingT4OrT6stove || total_load_before > 0 || total_load_qty > 0 || CbShift.Checked)
            {
                if (!int.TryParse(RC_NO_INFO["CURRENT_QTY"].ToString(), out current_qty) ||
                    total_load_qty + total_load_before != current_qty)
                {
...
";
            ...
            if (CbShift.Checked)
            {
                ...
                // 加工數量
                current_qty = int.Parse(RC_NO_INFO["CURRENT_QTY"].ToString());
