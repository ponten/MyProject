            ...
            try
            {
                ...
                if (MachineList.Count > 0)
                {
                    foreach (var rc in ExecuteRCs)
                    {
                        ...
                        List<MachineDownModel> using_machines = rc.Machines.Where(x => x.Select).ToList();

                        foreach (var m in using_machines)
                        {
                            // 負載量
                            if (m.LOAD_QTY < 0)
                            {
                        ...
                        if (using_machines.Count == 1 && i_load_qty == 0)
                        {
                            using_machines[0].LOAD_QTY = rc.CURRENT_QTY;

                            i_load_qty = rc.CURRENT_QTY;
                        }
                ...
                if (toPrintLabel)
                {
                    ...
                    {
                        if (!PlSrv.PrintLabel(ExecuteRCs, out message))
                        {
                            SajetCommon.Show_Message(message, 1);
                        }
                    }
