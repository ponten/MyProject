...
namespace RCIPQC
{
    public partial class fRemoveMachine : Form
    {
        ...
        private void FRemoveMachine_Load(object sender, EventArgs e)
        {
            ...
            LbInfo.Text = $"[{Machine.MACHINE_CODE}] {Machine.MACHINE_DESC}";
        ...
        private void BtnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!int.TryParse(TbLoad.Text, out int load_qty))
                {
                    message = SajetCommon.SetLanguage("The load of the machine must be a number");
                ...
                /*
                if (usingT4OrT6stove && load_qty <= 0)
                {
                    message = SajetCommon.SetLanguage("The load of the machine must be a greater-than-zero number");
...
";
        ...
        private void RemoveMachine()
        {
            var p = new List<object[]>
            {
                ...
                new object[] { ParameterDirection.Input, OracleType.VarChar, "load_qty", TbLoad.Text },
...
SET
    ...
    work_time_second = nvl(round(to_number(:end_time - start_time) * 24 * 60 * 60, 0), 3),
