        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (UpdateTypeEnum)
                {
                    ...
                    case UpdateType.Modify:
                        ModifyData();
                        UpdateRouteData();//SAJET.SYS_PART_ROUTE
                        DialogResult = DialogResult.OK;
                        break;
...
";
        ...
        private void CombRoute_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var current_route_row
                = dgvRoute.Rows.Cast<DataGridViewRow>()
                .FirstOrDefault(x => x.Cells["ROUTE_NAME"].Value.ToString() == route_name);

            if (current_route_row.Tag != null && current_route_row.Tag is SysEndProcessModel m)
            {
                TbEndProcess.Text = m.PROCESS_NAME;
            }
            else
            {
                TbEndProcess.Text = EpSrv.GetCurrentEndProcess(PART_ID: g_sKeyID ?? string.Empty, ROUTE_NAME: route_name);
            }
        }
        ...
        private bool HavaRequiredValues()
        {
            ...
            #region 棧板用量(單位用量/OPTION1)非必填，給預設值為 0，展工單時再決定 (2.0.17003.13)
        ...
        /// 把報工結束製程的設定儲存起來
        /// </summary>
        private void AppendEndProcess()
        {
            g_sPartID = SajetCommon.GetMaxID("SAJET.SYS_PART", "PART_ID", 10);

            foreach (DataGridViewRow row in dgvRoute.Rows)
            {
                if (row.Tag != null && row.Tag is SysEndProcessModel m)
                {
                    EpSrv.SaveEndProcessSetting(PART_ID: g_sPartID, ROUTE_ID: m.ROUTE_ID, node: m);
                }
            }
        }
...
";
        ...
        private void TsmiRouteDetail_Click(object sender, EventArgs e)
        {
            ...
            bool found_end_process_setting_in_model = false;

            string node_id_setting_in_model = "0";

            if (dgvRoute.CurrentRow.Tag != null &&
                dgvRoute.CurrentRow.Tag is SysEndProcessModel m)
            {
                found_end_process_setting_in_model = true;

                node_id_setting_in_model = m.NODE_ID;
            }
...
WITH ROUTE_DETAIL AS (
    ...
    WHERE
        ...
        AND ENABLED = 'Y'
{(found_end_process_setting_in_model ? "/**/" : "/*")}
    UNION
    SELECT
        {node_id_setting_in_model} NODE_ID
    FROM
        DUAL
{(found_end_process_setting_in_model ? "/**/" : "*/")}
)
...
";
            ...
            using (var f = new SajetFilter.FFilter(sqlCommand: s, @params: p, readOnly: readOnly, checkBox: true, checkBoxColumnName: name, advancedSearch: false, hiddenColumns: h))
            {
                ...
                {
                    dgvRoute.CurrentRow.Tag = null;
                }

                //TbEndProcess.Text = EpSrv.GetCurrentEndProcess(PART_ID: g_sKeyID);

                CombRoute_SelectedIndexChanged(null, null);
            }
        }
        ...
        {
            string from_part_id = dataCurrentRow.Cells["PART_ID"].Value.ToString().Trim();
            string userID = ClientUtils.UserPara1;
            DateTime now = OtSrv.GetDBDateTimeNow();

            var p = new List<object[]>
            {
                new object[] { ParameterDirection.Input, OracleType.Number, "FROM_PART_ID", from_part_id },
            };
...
WHERE
    PART_ID = :FROM_PART_ID
";

            var d = ClientUtils.ExecuteSQL(SQL, p.ToArray());
...
INSERT INTO SAJET.SYS_PART_QC_PLAN (
    ...
    :ENABLED
                    ...
                    p = new List<object[]>
                    {
                        ...
                        new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", row["PROCESS_ID"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.Number, "SAMPLING_ID", row["SAMPLING_ID"].ToString() },
                        ...
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ENABLED", "N" },
                    };

                    ClientUtils.ExecuteSQL(SQL, p.ToArray());
                }
            }
...
";
            ...
            d = ClientUtils.ExecuteSQL(SQL, p.ToArray());

            if (d != null && d.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in d.Tables[0].Rows)
                {
                    string MaxItemID = SajetCommon.GetMaxID("SAJET.SYS_PART_QC_ITEM", "ITEM_ID", 8);
...
INSERT INTO SAJET.SYS_PART_QC_ITEM (
    ...
    :ENABLED
                    ...
                    p = new List<object[]>
                    {
                        new object[] { ParameterDirection.Input, OracleType.Number, "PART_ID", PART_ID },
                        new object[] { ParameterDirection.Input, OracleType.Number, "PROCESS_ID", row["PROCESS_ID"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.Number, "ITEM_ID", MaxItemID },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ITEM_NAME", row["ITEM_NAME"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ITEM_TYPE", row["ITEM_TYPE"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ITEM_PHASE", row["ITEM_PHASE"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.Number, "ITEM_SEQ", row["ITEM_SEQ"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "INPUT_TYPE", row["INPUT_TYPE"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "VALUE_TYPE", row["VALUE_TYPE"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "VALUE_DEFAULT", row["VALUE_DEFAULT"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "VALUE_LIST", row["VALUE_LIST"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "CONVERT_TYPE", row["CONVERT_TYPE"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "COLUMN_ITEM", row["COLUMN_ITEM"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ROW_ITEM", row["ROW_ITEM"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "NECESSARY", row["NECESSARY"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "PRINT", row["PRINT"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.Number, "UNIT_ID", row["UNIT_ID"].ToString() },
                        new object[] { ParameterDirection.Input, OracleType.Number, "UPDATE_USERID", userID },
                        new object[] { ParameterDirection.Input, OracleType.DateTime, "UPDATE_TIME", now },
                        new object[] { ParameterDirection.Input, OracleType.VarChar, "ENABLED", row["ENABLED"].ToString() },
                    };
...
";
            p = new List<object[]>
            {
                new object[] { ParameterDirection.Input, OracleType.Number, "TO_PART_ID", PART_ID },
                new object[] { ParameterDirection.Input, OracleType.Number, "FROM_PART_ID", from_part_id },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION1", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION2", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION3", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION4", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION5", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION6", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION7", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION8", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION9", "N" },
                new object[] { ParameterDirection.Input, OracleType.VarChar, "OPTION10", "N" },
                new object[] { ParameterDirection.Input, OracleType.Number, "UPDATE_USERID", userID },
                new object[] { ParameterDirection.Input, OracleType.DateTime, "UPDATE_TIME", now },
            };
